...
model = dict(
    ...
    backbone=dict(
        ...
        init_cfg=dict(
            type='Pretrained',
            checkpoint='https://download.openmmlab.com/mmclassification/v0/swin-transformer/swin_small_patch4_window7_224.pth'
        )
    ),
    neck=dict(
        type='FPN',
    ...
    rpn_head=dict(
        ...
        bbox_coder=dict(
            ...
            target_means=[0.0, 0.0, 0.-leaks0, 0.0],
...
train_pipeline = [
    ...
    dict(type='RandomRotate', angle=(-30, 30), prob=0.5),
    dict(type='PhotoMetricDistortion', brightness_delta=32, contrast_range=(0.5, 1.5)),
    dict(
        type='Normalize',
...
data = dict(
    ...
    test=dict(
        ...
        img_prefix=f'{data_img_prefix}/',
        pipeline=test_pipeline,
        classes=('Ascaris lumbricoides', 'Capill```python
...
log_level = 'INFO'
load_from = 'https://download.openmmlab.com/mmdetection/v2.0/htc/htc_swin-s_p4_w7_fpn_20e_coco/htc_swin-s_p4_w7_fpn_20e_coco_20210709_133054-8a528ccc.pth'
resume_from = None
workflow = [('train', 1)]
...
work_dir = '/kaggle/working/htc_swin-s_p4_w7_fpn_20e_coco_chulacell'
auto_resume = False
gpu_ids = range(0, 2)